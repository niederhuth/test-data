#! /usr/bin/perl
# reformat_exonerate_protein_gff.pl
# This will reformat the exonerate protein alignments so that they will display correctly in IGV.
# 19 March 2021
# Kevin Childs
...
if (!defined($input_gff) ||  !(-e $input_gff) || !defined($output_gff) ||  (-e $output_gff)) {
    die $usage;
}
...
while (my $line = <OLD_GFF>) {
    ...
    #
    if ($elems[2] eq 'gene') { $elems[2] = "mRNA"; }
    #
    if ($elems[2] eq 'exon') { $elems[2] = "exon"; }
    #
    $elems[1] = "protein2genome";
    #
    if ($elems[2] eq 'mRNA') {
        ++$id;
        my $name = "unknown";
